...
2. **[FASE 2 — Paralelo]** al aprobar la spec:
   - `Backend Developer` → implementa `plataforma-danos-back/`
   - `Frontend Developer` → implementa `cotizador-danos-web/`
   - `Database Agent` → si hay cambios de esquema en la spec
3. **[FASE 3 — Paralelo]** al completar implementación:
   - `Test Engineer Backend` → genera `plataforma-danos-back/src/test/java/`
   - `Test Engineer Frontend` → genera `cotizador-danos-web/src/__tests__/`
