...
> Adaptación actual del framework: este repositorio se está preparando para un **monorepo** del reto de **cotizador de daños** con módulos `cotizador-danos-web`, `plataforma-danos-back`, `plataforma-core-ohs`, `automatizacion` y `docs/`.
...
|---|---------|-------------|
| 1 | `README.md` (raíz del proyecto) | Stack, arquitectura, comandos (`build`, `test`, `run`), variables de entorno y módulos del monorepo |
| 2 | `.github/copilot-instructions.md` | Diccionario de dominio, DoR, DoD y mapa metodológico del proyecto |
| 3 | `.github/instructions/backend.instructions.md` | Stack backend, arquitectura, convenciones y wiring del proyecto |
| 4 | `.github/instructions/frontend.instructions.md` | Stack frontend, rutas, estado y convenciones UI |
| 5 | `.github/instructions/tests.instructions.md` | Estrategia de pruebas, herramientas y cobertura mínima |

Una vez completados, los agentes tienen todo el contexto para operar de forma autónoma.

**No modificar sin intención explícita**: `agents/`, `.github/docs/lineamientos/`.

**Sí deben adaptarse por proyecto**: `.github/copilot-instructions.md`, `.github/instructions/`, `.github/prompts/`, `README.md`, `.github/requirements/` y `.github/specs/`.
...
|---|---|
| `plataforma-danos-back/src/main/java/**/*.java` (o equivalente) | `instructions/backend.instructions.md` |
| `cotizador-danos-web/src/**/*.{js,jsx}` (o equivalente) | `instructions/frontend.instructions.md` |
| `plataforma-danos-back/src/test/**/*.java` / `cotizador-danos-web/src/__tests__/**` / `automatizacion/**` | `instructions/tests.instructions.md` |
...
Project Root/
│
├── cotizador-danos-web/            ← SPA React/Vite
├── plataforma-danos-back/          ← backend Spring Boot + PostgreSQL
├── plataforma-core-ohs/            ← servicio core mock / stub
├── automatizacion/                 ← pruebas E2E Serenity BDD + Screenplay
...
└── .github/                         ← framework Copilot (auto-contenido para compartir)
    ...
    │   ├── backend.instructions.md  ← applyTo: plataforma-danos-back/**
    │   ├── frontend.instructions.md ← applyTo: cotizador-danos-web/src/**
    │   └── tests.instructions.md    ← applyTo: src/test/**, __tests__/**, automatizacion/**
    │
...
El enunciado del reto menciona `ASSD`; el framework y sus artefactos internos utilizan `ASDD` (**Agent Spec-Driven Development**). Para evitar bifurcar la metodología, la documentación del proyecto debe mantener `ASDD` y aclarar esta equivalencia en el README principal de la entrega.
