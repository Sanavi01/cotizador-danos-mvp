...
PARA APLICACIONES WEB (E2E):
  Serenity BDD + Screenplay → Primera opción para este reto y módulo `automatizacion`
  Playwright → Alternativa válida si el proyecto cambia de estándar
  Cypress    → Si ya existe en el proyecto

PARA APIs REST (sin UI):
  JUnit + Rest Assured → Para backend Java/Spring Boot
